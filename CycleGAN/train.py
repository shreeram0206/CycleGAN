...
## Make required changes and also experiment with different model hyperparameters and architectures
...
def train_fn(disc_M, disc_R, gen_R, gen_M, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler):
    ...
    for idx, (photo, monet) in enumerate(loop):
        ...
        # print("Monet real images loss item is", M_reals)
        # print("Monet real images loss item divided by index is", M_reals/(idx+1))
    ...
    # print(G_loss.item())
    # print("the final loss after completion of the epoch is", loss_value)
    return G_loss, D_loss


# def plot_loss(epochs_array, loss_array, y_axis, title):
#     plt.plot(epochs_array, loss_array)
#     plt.xlabel("Epochs")
#     plt.ylabel("{0}".format(y_axis))
#     plt.title("{0}_Loss vs Number of Epochs".format(title))
#     plt.show()
#     plt.savefig("Loss_plot_{0}.png".format(title))
...
def main():
    ...
    for epoch in range(config.NUM_EPOCHS):
        g_loss, disc_loss = train_fn(disc_M, disc_R, gen_R, gen_M, val_loader, opt_disc, opt_gen, L1, mse, d_scaler, g_scaler)
        ...
        print("Plotting loss graph after epoch {0}".format(epoch))
        epochs_array.append(epoch)
        # plot_loss(epochs_array, G_loss_array, y_axis="Total Loss", title="Total")
        plot1 = plt.figure(1)
        ...
        plt.show()
        # plot_loss(epochs_array, discriminator_loss_array, y_axis="Discriminator Loss", title="Discriminator")
